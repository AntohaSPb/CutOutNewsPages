...
norm_proc = 'numpy_minmax' # or 'torch' or 'numpy_stdev' or 'none'

in_folder = 'c:/Pyproj/Segm/Code/imagery/Test2'  # Replace with your image path
out_folder = 'c:/Pyproj/Segm/Code/imagery/Test2/Out'  # Replace with your output path

need_show = True

if used_model == 'Unet':
    #inference code for U-Net
    best_model = torch.jit.load('c:/Pyproj/Segm/Code/models/best_model_new.pt', map_location='cpu')
...
for filename in os.listdir(in_folder):
    ...
    im_path = os.path.join(in_folder, filename)
    ou_path = os.path.join(out_folder, filename)

    #im_path = 'c:/Pyproj/Segm/Code/imagery/Test/Source.jpg'  # Replace with your image path
    ...
    if used_model == 'Resnet':
        ...
        sel_mask = np.argmax(np_mask, axis=0)
        page_size = sel_mask.shape
        print ("likely mask size", page_size)
        #convert mask for manipulations
        likely_mask = sel_mask.astype(np.uint8)

    if used_model == 'Unet':
        # kill batch dim=1, detach tensor, shift classes to axis=2, turn into numpy array
        np_mask = pred_mask.squeeze().permute(1, 2, 0).numpy()
        #select my mask class
        sel_mask = np_mask[:, :, 0]  # shape (256, 384)
        #filter out pixels having below 50% prob of req obj type (i.e. all wrong objects)
        likely_mask = (sel_mask > 0.9).astype(np.uint8) * 255  # shape (256, 384)
    ...
    success = cv2.imwrite(ou_path, ppage_combi)
    # Optional: Check if the image was saved successfully
    if success:
        print(f'Image saved successfully at {ou_path}')
    else:
        print('Failed to save the image.')

    del ppage, ppage_norm, likely_mask, np_mask, ppage_small, ppageRGB, ppage_tensor, ppage_combi, sel_mask
